defmodule PollutionData do
  @moduledoc false
  def loadModules() do
    IEx.Helpers.c("../lab4/src/pollution_gen_server_backup.erl")
    IEx.Helpers.c("../lab4/src/pollution_gen_server.erl")
    IEx.Helpers.c("../lab4/src/pollution_gen_server_sup.erl")
    IEx.Helpers.c("../lab2/src/pollution.erl")
  end
  ...
  def addAllStations(map) do
    ...
    map = Enum.map(Map.to_list(map), fn {{x,y}, :station} -> {"station_#{x}_#{y}",{x,y}} end)
    fn -> Enum.map(map, fn {name, location} -> :pollution_gen_server.addStation(name, location) end) end |> :timer.tc |> elem(0)
  end
  def addAllValues(list) do
    fn -> Enum.map(list,
      fn %{:datetime => datetime, :location => location, :pollutionLevel => pollutionLevel}
      -> :pollution_gen_server.addValue(location, datetime, "PM10", pollutionLevel) end) end |> :timer.tc |> elem(0)
  end
  def addCSVData() do
    loadModules()
