...
%%%
%%% @end
%%% Created : 06. maj 2018 20:03
%%%-------------------------------------------------------------------
-module(pollution_gen_server_sup_tests).
...
start_test() ->
  ...
  {ok,Pid} = pollution_gen_server_sup:start(),
  Pid = whereis(pollution_gen_server_sup).
addStation1_test() ->
...
addValue2_test() ->
  no_such_station = pollution_gen_server:addValue({12, 35}, {{2018,4,1}, {0,0,0}}, temp, 10),
  [{measurement, {{2018,4,1},{0,0,0}}, temp, 10}] = dict:fetch({station, "Qwerty", {12, 34}}, pollution_gen_server:getMonitor()).
addValue3_test() ->
  already_exists = pollution_gen_server:addValue("Qwerty", {{2018,4,1}, {0,0,0}}, temp, 20),
...
removeValue1_test() ->
  ...
  3 = length(dict:fetch({station, "Qwerty", {12, 34}}, pollution_gen_server:getMonitor())).
removeValue2_test() ->
  no_such_station = pollution_gen_server:removeValue("Qwertyasdfgh", {{2018,4,1}, {12,0,0}}, temp),
...
getStationMean3_test() ->
  ok = pollution_gen_server:addValue({12, 34}, {{2018,4,2}, {0,0,0}}, "PM10", 39),
...
stop_test() ->
  true = pollution_gen_server_sup:stop().
stop2_test() ->
  undefined = whereis(pollution_gen_server),
  undefined = whereis(pollution_gen_server_sup),
  undefined = whereis(pollution_gen_server_backup).
